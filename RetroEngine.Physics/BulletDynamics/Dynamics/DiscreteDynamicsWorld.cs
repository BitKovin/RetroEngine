...
namespace BulletXNA.BulletDynamics
{
    public class DiscreteDynamicsWorld : DynamicsWorld
    {
        ...
        public override void SetGravity(ref IndexedVector3 gravity)
        {
            ...
            {
                RigidBody body = m_nonStaticRigidBodies[i];
                if (body.IsActive() && ((body.GetFlags() & RigidBodyFlags.DisableWorldGravity) == 0))
                {
        ...
        {
            if (!body.IsStaticOrKinematicObject() && 0 == (body.GetFlags() & RigidBodyFlags.DisableWorldGravity))
            {
            ...
            if (body.GetCollisionShape() != null)
            {
                ...
                {
                    body.SetActivationState(ActivationState.IslandSleeping);
                }
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                if (body != null)
                {
                    ...
                    if (body.WantsSleeping())
                    {
                        ...
                        {
                            if (body.GetActivationState() == ActivationState.ActiveTag)
                            {
                                body.SetActivationState(ActivationState.WantsDeactivation);
                            }

                            if (body.GetActivationState() == ActivationState.IslandSleeping)
                            {
                    ...
                    {
                        if (body.GetActivationState() != ActivationState.DisableDeactivation)
        ...
        protected virtual void SaveKinematicState(float timeStep)
        {
            ...
            {
                RigidBody body = RigidBody.Upcast(m_collisionObjects[i]);
                if (body != null && body.GetActivationState() != ActivationState.IslandSleeping)
                {

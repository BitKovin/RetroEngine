        ...
        public void BuildIslands(IDispatcher dispatcher, CollisionWorld collisionWorld)
        {
            ...
            for (int startIslandIndex = 0; startIslandIndex < numElem; startIslandIndex = endIslandIndex)
            {
                ...
                for (int idx = startIslandIndex; idx < endIslandIndex; idx++)
                {
                    ...
                    {
                        if (colObj0.GetActivationState() == ActivationState.ActiveTag)
                        {
                            allSleeping = false;
                        }
                        if (colObj0.GetActivationState() == ActivationState.DisableDeactivation)
                        {
                            allSleeping = false;
                        }
                    }
                ...
                if (allSleeping)
                {
                    for (int idx = startIslandIndex; idx < endIslandIndex; idx++)
                    {
                        ...
                        {
                            colObj0.SetActivationState(ActivationState.IslandSleeping);
                        }
                    }
                }
                else
                {
                    for (int idx = startIslandIndex; idx < endIslandIndex; idx++)
                    {
                        ...
                        if (islandTag == islandId)
                        {
                            if (colObj0.GetActivationState() == ActivationState.IslandSleeping)
                            {
                                colObj0.SetActivationState(ActivationState.WantsDeactivation);
            ...
            for (int i = 0; i < maxNumManifolds; i++)
            {
                ...
                if (((colObj0 != null) && colObj0.GetActivationState() != ActivationState.IslandSleeping) ||
                   ((colObj1 != null) && colObj1.GetActivationState() != ActivationState.IslandSleeping))
                {

                    //kinematic objects don't merge islands, but wake up all connected objects
                    if (colObj0.IsKinematicObject() && colObj0.GetActivationState() != ActivationState.IslandSleeping)
                    {
                    ...
                    if (colObj1.IsKinematicObject() && colObj1.GetActivationState() != ActivationState.IslandSleeping)
                    {

...
namespace RetroEngine.Skeletal
{
    ...
    public class RiggedAlignedNormalArrows
    {
        ...
        public void CreateVisualNormalsForPrimitiveMesh(VertexData[] inVertices, int[] inIndices, Texture2D t, float thickness, float scale)
        {
            texture = t;
            int len = inVertices.Length;

            // for each vertice in the model we will make a quad composed of 4 vertices and 6 indices.
            VertexData[] nverts = new VertexData[len * 4];
    ...
    public class NormalArrow
    {
        ...
        public NormalArrow(VertexData[] inVertices, int[] inIndices, Texture2D t, float scale)
        {
            VertexPositionNormalTexture[] v = new VertexPositionNormalTexture[inVertices.Length];
            int[] i = new int[inIndices.Length];
            for (int n = 0; n < inVertices.Length; n++)
